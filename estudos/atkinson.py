...
class Bitmap(object):
	'''not an image, but implements putpixel and getpixel like one'''
...
def make_bitmap(width, height, gradient_height):
...
def make_image(width, height, gradient_height):
	import PIL.Image
	bitmap = make_bitmap(width, height, gradient_height)
	img = PIL.Image.new('L', bitmap.size, 0)
	img.putdata(list(bitmap))
	return img	
